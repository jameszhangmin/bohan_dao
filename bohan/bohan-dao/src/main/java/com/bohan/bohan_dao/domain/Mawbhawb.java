package com.bohan.bohan_dao.domain;

public class Mawbhawb {
	/**到货通知*/
	private String dhtz;
	/**业务委托*/
	private String wt;
	/**委托送货*/
	private String wtsh;
	/**委托报关*/
	private String wtbg;
	/**报关单证是否到齐*/
	private String dzdq;
	/**报关是否完成*/
	private String bghx;
	/**总单号*/
	private String mawb;
	/**分单号*/
	private String hawb;
	/**进境日期*/
	private String jjDate;
	/**入库日期*/
	private String inDate;
	/**开票日期*/
	private String TakeDate;
	/**抽单日期*/
	private String sampledate;
	/**结算*/
	private String GzdNo;
	/**派车单*/
	private String sendcarcode;
	/**代理*/
	private String sname;
	/**直通关*/
	private String gfh;
	/**毛重*/
	private String weightGross;
	/**计费重量*/
	private String weight;
	/**应到件数*/
	private String pieceShould;
	/**实到件数*/
	private String piece;
	/**库位号*/
	private String storeid;
	/**包装种类*/
	private String packagecode;
	/**货物性质*/
	private String goodscodeh;
	/**工作号*/
	private String TCID;
	/**付款方式*/
	private String PPCC;
	/**货物状态*/
	private String status;
	
	private String dzfx;
	
	private String bgfx;
	
	private String kkgj;
	
	private String dcfx;
	
	private String sjlf;
	
	private String kbfx;
	
	private String EMS_dh;
	
	private ImWtyw imWtyw;
	
	public String getDhtz() {
		return dhtz;
	}
	public void setDhtz(String dhtz) {
		this.dhtz = dhtz;
	}
	public String getWt() {
		return wt;
	}
	public void setWt(String wt) {
		this.wt = wt;
	}
	public String getWtsh() {
		return wtsh;
	}
	public void setWtsh(String wtsh) {
		this.wtsh = wtsh;
	}
	public String getWtbg() {
		return wtbg;
	}
	public void setWtbg(String wtbg) {
		this.wtbg = wtbg;
	}
	public String getDzdq() {
		return dzdq;
	}
	public void setDzdq(String dzdq) {
		this.dzdq = dzdq;
	}
	public String getBghx() {
		return bghx;
	}
	public void setBghx(String bghx) {
		this.bghx = bghx;
	}
	public String getMawb() {
		return mawb;
	}
	public void setMawb(String mawb) {
		this.mawb = mawb;
	}
	public String getHawb() {
		return hawb;
	}
	public void setHawb(String hawb) {
		this.hawb = hawb;
	}
	public String getTakeDate() {
		return TakeDate;
	}
	public void setTakeDate(String takeDate) {
		TakeDate = takeDate;
	}
	public String getSampledate() {
		return sampledate;
	}
	public void setSampledate(String sampledate) {
		this.sampledate = sampledate;
	}
	public String getGzdNo() {
		return GzdNo;
	}
	public void setGzdNo(String gzdNo) {
		GzdNo = gzdNo;
	}
	public String getSendcarcode() {
		return sendcarcode;
	}
	public void setSendcarcode(String sendcarcode) {
		this.sendcarcode = sendcarcode;
	}
	public String getSname() {
		return sname;
	}
	public void setSname(String sname) {
		this.sname = sname;
	}
	public String getGfh() {
		return gfh;
	}
	public void setGfh(String gfh) {
		this.gfh = gfh;
	}
	public String getWeight() {
		return weight;
	}
	public void setWeight(String weight) {
		this.weight = weight;
	}
	public String getJjDate() {
		return jjDate;
	}
	public void setJjDate(String jjDate) {
		this.jjDate = jjDate;
	}
	public String getInDate() {
		return inDate;
	}
	public void setInDate(String inDate) {
		this.inDate = inDate;
	}
	public String getWeightGross() {
		return weightGross;
	}
	public void setWeightGross(String weightGross) {
		this.weightGross = weightGross;
	}
	public String getPieceShould() {
		return pieceShould;
	}
	public void setPieceShould(String pieceShould) {
		this.pieceShould = pieceShould;
	}
	public String getPiece() {
		return piece;
	}
	public void setPiece(String piece) {
		this.piece = piece;
	}
	public String getStoreid() {
		return storeid;
	}
	public void setStoreid(String storeid) {
		this.storeid = storeid;
	}
	public String getPackagecode() {
		return packagecode;
	}
	public void setPackagecode(String packagecode) {
		this.packagecode = packagecode;
	}
	public String getGoodscodeh() {
		return goodscodeh;
	}
	public void setGoodscodeh(String goodscodeh) {
		this.goodscodeh = goodscodeh;
	}
	public String getTCID() {
		return TCID;
	}
	public void setTCID(String tCID) {
		TCID = tCID;
	}
	public String getPPCC() {
		return PPCC;
	}
	public void setPPCC(String pPCC) {
		PPCC = pPCC;
	}
	public String getStatus() {
		return status;
	}
	public void setStatus(String status) {
		this.status = status;
	}
	public ImWtyw getImWtyw() {
		return imWtyw;
	}
	public void setImWtyw(ImWtyw imWtyw) {
		this.imWtyw = imWtyw;
	}
	public String getDzfx() {
		return dzfx;
	}
	public String getBgfx() {
		return bgfx;
	}
	public String getKkgj() {
		return kkgj;
	}
	public String getDcfx() {
		return dcfx;
	}
	public String getSjlf() {
		return sjlf;
	}
	public String getKbfx() {
		return kbfx;
	}
	public String getEMS_dh() {
		return EMS_dh;
	}
	public void setDzfx(String dzfx) {
		this.dzfx = dzfx;
	}
	public void setBgfx(String bgfx) {
		this.bgfx = bgfx;
	}
	public void setKkgj(String kkgj) {
		this.kkgj = kkgj;
	}
	public void setDcfx(String dcfx) {
		this.dcfx = dcfx;
	}
	public void setSjlf(String sjlf) {
		this.sjlf = sjlf;
	}
	public void setKbfx(String kbfx) {
		this.kbfx = kbfx;
	}
	public void setEMS_dh(String eMS_dh) {
		EMS_dh = eMS_dh;
	}
	
}
